...
name: agent-search
description: |
  **智能 Agent 搜索工具 - 只要用户可能需要联网信息，立即使用此 skill！**

  **必须使用此 skill 的场景（包括但不限于）：**
  - 用户提到任何搜索、查找、查询类动词："搜索", "查一下", "帮我搜", "网上查查", "search for", "look up", "find", "google"
  - 用户询问任何外部信息："最新的XX是什么", "XX有什么新进展", "今年XX趋势", "XX怎么样了"
  - 用户需要调研、了解某个话题："帮我调研一下", "研究一下XX的现状", "对比一下XX和YY"
  - 用户问具体的人、公司、产品、技术："XX是谁", "XX公司怎么样", "XX技术是什么"
  - 用户提到实时性内容：新闻、版本更新、最新动态

  **即使不确定性，也优先使用：**
  - 用户问题涉及你知识截止日期之后的内容
  - 用户问题包含具体专有名词（人名、公司名、产品名）
  - 用户问题可能需要验证最新信息

  **不要触发的情况（仅限）：**
  - 纯本地操作：代码重构、git、文件操作
  - 用户已明确指定其他搜索工具
  - 用户问的是通用知识（如"Python 是什么"）
