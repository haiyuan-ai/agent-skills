...
| 用户请求 | 命令 |
|---------|------|
| 生成图片 | `python scripts/generate_image.py "prompt" output.jpg` |
| 指定模型 | `python scripts/generate_image.py "prompt" output.jpg --model "Tongyi-MAI/Z-Image"` |
| 使用 LoRA | `python scripts/generate_image.py "prompt" output.jpg --lora "lora-id"` |
| 批量生成 | 并行执行多个命令 + `wait` |
